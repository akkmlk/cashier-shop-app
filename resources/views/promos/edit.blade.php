...
@section('content')
    <div class="row">
        <div class="col-xl-4 col-lg-6">
            <form action="{{ route('promo.update', $promo->id) }}" method="POST" class="card card-orange card-outline">
                @csrf
                @method('PUT')
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <x-input name="name" type="text" placeholder="Nama" value="{{ $promo->name }}"/>
                    </div>
                    <div class="form-group">
                        <label for="">Deskripsi</label>
                        <textarea name="description" cols="25" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $promo->description) }}</textarea>
                        @error('description')
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control @error('type') is-invalid @enderror" id="type" >
                            @foreach ($typePromo as $option)
                                <option value="{{ old('type', $option['value']) }}" {{ $option['value'] == $promo->type ? 'selected' : '' }}>{{ $option['option'] }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group">
                        ...
                        @php
                            $buy = [
                                ['value' => '', 'option' => 'Pilih jumlah beli'],
                                ['value' => 1, 'option' => 1],
                                ['value' => 2, 'option' => 2],
                                ['value' => 3, 'option' => 3],
                                ['value' => 4, 'option' => 4],
                        ...
                        <select name="buy" class="form-control @error('buy') is-invalid @enderror" id="buy" disabled >
                            @foreach ($buy as $option)
                                <option value="{{ old('buy', $option['value']) }}" {{ $option['value'] == $promo->buy ? 'selected' : '' }}>{{ $option['option'] }}</option>
                            @endforeach
                        </select>
                        @error('buy')
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group">
                        ...
                        <select name="get" class="form-control @error('get') is-invalid @enderror" id="get" disabled >
                            @foreach ($buy as $option)
                                <option value="{{ old('get', $option['value']) }}" {{ $option['value'] == $promo->get ? 'selected' : '' }}>{{ $option['option'] }}</option>
                            @endforeach
                        </select>
                        @error('get')
                            <div class="invalid-feedback">
                    ...
                    <div class="form-group">
                        ...
                        {{-- <x-input name="discount" type="number" value="{{ $promo->discount }}" /> --}}
                        <input type="number" id="discount" name="discount" class="form-control @error('discount') is-invalid @enderror" value="{{ old('discount', $promo->discount) }}" disabled>
                        @error('discount')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <x-select name="active" :options="$status" value="{{ $promo->active }}" />
                    </div>
                    <div class="form-group">
                        <label for="">Product nya</label>
                        <div class="input-group">
                            <input type="text" class="form-control" id="productNames" value="{{ $stringSelectedNameProducts }}" placeholder="Pilih product nya" readonly>
                            <input type="hidden" class="form-control" name="products" id="products" value="{{ $stringSelectedIdProducts }}" placeholder="Pilih product nya" readonly>
